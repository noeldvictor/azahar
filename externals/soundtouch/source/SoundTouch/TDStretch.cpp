...
#if defined(SOUNDTOUCH_USE_NEON) && defined(__aarch64__)
#include <arm_neon.h>
#endif
...
{
#if defined(SOUNDTOUCH_USE_NEON) && defined(__aarch64__)
    // Integer SoundTouch always rounds the overlap length to a power of two. Process four stereo
    // frames at a time and replace the scalar division in every channel with an exact signed shift.
    // The bias preserves C++ division's truncation toward zero for negative sample sums.
    const int divisionShift = overlapDividerBitsPure + 1;
    assert(overlapLength == (1 << divisionShift));
    assert((overlapLength & 3) == 0);

    const int32x4_t roundingBias = vdupq_n_s32(overlapLength - 1);
    const int32x4_t rightShift = vdupq_n_s32(-divisionShift);
    const int16x8_t overlap = vdupq_n_s16((short)overlapLength);
    const int16x8_t four = vdupq_n_s16(4);
    int16x8_t inputWeight = {0, 0, 1, 1, 2, 2, 3, 3};

    const auto divideTowardZero = [roundingBias, rightShift](int32x4_t value)
    {
        const int32x4_t negativeBias = vandq_s32(vshrq_n_s32(value, 31), roundingBias);
        return vshlq_s32(vaddq_s32(value, negativeBias), rightShift);
    };

    for (int i = 0; i < overlapLength; i += 4)
    {
        const int16x8_t inputSamples = vld1q_s16(input + 2 * i);
        const int16x8_t midSamples = vld1q_s16(pMidBuffer + 2 * i);
        const int16x8_t midWeight = vsubq_s16(overlap, inputWeight);

        int32x4_t lower = vmull_s16(vget_low_s16(inputSamples), vget_low_s16(inputWeight));
        lower = vmlal_s16(lower, vget_low_s16(midSamples), vget_low_s16(midWeight));
        int32x4_t upper = vmull_high_s16(inputSamples, inputWeight);
        upper = vmlal_high_s16(upper, midSamples, midWeight);

        lower = divideTowardZero(lower);
        upper = divideTowardZero(upper);
        vst1q_s16(poutput + 2 * i, vcombine_s16(vmovn_s32(lower), vmovn_s32(upper)));

        inputWeight = vaddq_s16(inputWeight, four);
    }
#else
    int i;
    short temp;
