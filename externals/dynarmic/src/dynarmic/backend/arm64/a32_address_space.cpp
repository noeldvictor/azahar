...
void A32AddressSpace::InvalidateCacheRanges(const boost::icl::interval_set<u32>& ranges) {
    const auto descriptors = block_ranges.InvalidateRanges(ranges);
    for (const auto descriptor : descriptors) {
        auto& entry = GetFastDispatchEntry(descriptor);
        if (entry.location_descriptor == descriptor.Value()) {
            entry = {};
        }
    }
    InvalidateBasicBlocks(descriptors);
}

void A32AddressSpace::ClearFastDispatchTable() {
...
void A32AddressSpace::EmitPrelude() {
    ...
    oaknut::Label return_from_run_code, l_return_to_dispatcher, l_fast_dispatch_table;
    ...
    prelude_info.fast_dispatch = code.xptr<void*>();
    {
        ...
        code.EOR(Xscratch0, X20, X20, LSR, 32);
        code.LSR(Xscratch0, Xscratch0, 1);
        code.AND(Xscratch0, Xscratch0, fast_dispatch_table_size - 1);
        code.LDR(Xscratch1, l_fast_dispatch_table);
        code.ADD(X19, Xscratch1, Xscratch0, LSL, 4);

        code.LDP(Xscratch0, Xscratch1, X19);
        code.CMP(X20, Xscratch0);
