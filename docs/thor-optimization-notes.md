...
- The optimization is gated to AArch64 and only to formats whose old behavior can be represented exactly: RGBA8 with or without its existing whole-pixel byte reversal, plus non-converted raw 16-bit formats. RGB8/D24 packing, D24S8 rotation, ETC decoding, and the intensity/alpha expansion formats stay on their existing scalar paths.
- A focused test covers both Morton-to-linear upload and linear-to-Morton download, a deliberately padded ten-pixel stride, native and converted RGBA8, all three native 16-bit color formats, and D16. It compares against a byte-wise scalar Morton reference and verifies exact round trips. It passed on the AYN Thor with 12 assertions in one test case.
- `:app:assembleVanillaRelWithDebInfoLite` passed in 2m26s. ThinLTO preserved the intended full-tile `LD2`/vector `REV32`/paired-store sequence in `libcitra-android.so`. The APK is 28,945,367 bytes with SHA-256 `912720382B7E76EC433BA47C014F5674CBF2CC730EA233004C97187C8D23FC35`.
