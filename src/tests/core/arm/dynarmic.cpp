...
TEST_CASE("Dynarmic A32 RBIT reverses every bit and preserves aliases, flags, and FPSCR",
          "[core][arm][dynarmic]") {
    struct Operation {
        std::uint32_t instruction;
        std::uint8_t destination;
    ...
    constexpr std::array operations{
        Operation{0xe6ff0f32, 0, 2, false}, // ARM RBIT R0, R2
        Operation{0xe6ff4f34, 4, 4, false}, // ARM RBIT R4, R4
        Operation{0xf0a2fa92, 0, 2, true},  // Thumb RBIT R0, R2
        Operation{0xf4a4fa94, 4, 4, true},  // Thumb RBIT R4, R4
    };
    constexpr std::array inputs{
        0x00000000u,
        0x00000001u,
    ...
    const auto reverse_bits = [](std::uint32_t value) {
        std::uint32_t result = 0;
        for (std::size_t bit = 0; bit < 32; ++bit) {
            result |= ((value >> bit) & 1U) << (31 - bit);
        }
        return result;
    };
    ...
    for (const auto& operation : operations) {
        ...
        for (const std::uint32_t input : inputs) {
            ...
            CHECK(jit.Regs()[operation.destination] == reverse_bits(input));
            for (std::size_t reg = 0; reg < 15; ++reg) {
                if (reg == operation.destination) {
                    continue;
                }
...
TEST_CASE("Dynarmic A32 REV16 swaps bytes in both halfwords and preserves aliases, flags, and FPSCR",
          "[core][arm][dynarmic]") {
    struct Operation {
        std::uint32_t instruction;
        std::uint8_t destination;
    ...
    constexpr std::array operations{
        Operation{0xe6bf0fb2, 0, 2, false}, // ARM REV16 R0, R2
        Operation{0xe6bf4fb4, 4, 4, false}, // ARM REV16 R4, R4
        Operation{0xe7feba50, 0, 2, true},  // Thumb-16 REV16 R0, R2; B .
        Operation{0xe7feba64, 4, 4, true},  // Thumb-16 REV16 R4, R4; B .
        Operation{0xf092fa92, 0, 2, true},  // Thumb-2 REV16 R0, R2
        Operation{0xf494fa94, 4, 4, true},  // Thumb-2 REV16 R4, R4
    };
    constexpr std::array inputs{
        0x00000000u,
        0x000000ffu,
    ...
    for (const auto& operation : operations) {
        ...
        for (const std::uint32_t input : inputs) {
            ...
            initial_regs[operation.source] = input;
            const std::uint32_t expected =
                ((input & 0x00ff00ffU) << 8) | ((input & 0xff00ff00U) >> 8);
...
TEST_CASE("Dynarmic A32 bitfield extract preserves signedness, aliases, flags, and FPSCR",
          "[core][arm][dynarmic]") {
    struct Field {
        std::uint8_t lsb;
        std::uint8_t width;
    ...
    std::array<Operation, 48> operations{};
    std::size_t operation_index = 0;
    for (const bool thumb : {false, true}) {
        for (const bool is_signed : {false, true}) {
            for (const bool source_alias : {false, true}) {
                ...
                for (const auto& field : fields) {
                    const std::uint8_t widthm1 = static_cast<std::uint8_t>(field.width - 1);
                    const std::uint32_t instruction = [&] {
                        if (!thumb) {
                            const std::uint32_t base = is_signed ? 0xe7a00050U : 0xe7e00050U;
                            return base | (static_cast<std::uint32_t>(widthm1) << 16) |
                                   (static_cast<std::uint32_t>(destination) << 12) |
                                   (static_cast<std::uint32_t>(field.lsb) << 7) | source;
                        }
                        const std::uint32_t first_half =
                            (is_signed ? 0xf340U : 0xf3c0U) | source;
                        const std::uint32_t second_half =
                            (static_cast<std::uint32_t>(field.lsb >> 2) << 12) |
                            (static_cast<std::uint32_t>(destination) << 8) |
                            (static_cast<std::uint32_t>(field.lsb & 3) << 6) | widthm1;
                        return (second_half << 16) | first_half;
                    }();
                    operations[operation_index++] = Operation{
                        instruction,
    ...
    for (const auto& operation : operations) {
        CAPTURE(operation.instruction, operation.destination, operation.source, operation.lsb,
                operation.width, operation.is_signed, operation.thumb);
        ArmTestCallbacks callbacks;
        callbacks.code = {
            operation.instruction,
            operation.thumb ? 0xe7fee7fe : 0xeafffffe, // B .
        ...
        for (const std::uint32_t input : inputs) {
            ...
            const std::uint32_t mask =
                operation.width == 32 ? 0xffffffffU : (1U << operation.width) - 1;
            std::uint32_t expected = (input >> operation.lsb) & mask;
            if (operation.is_signed && operation.width < 32 &&
                (expected & (1U << (operation.width - 1))) != 0) {
                expected |= ~mask;
            }
...
TEST_CASE("Dynarmic A32 shifted arithmetic preserves aliases, flags, and full-width wrapping",
          "[core][arm][dynarmic]") {
            ...
            for (const auto& layout : layouts) {
                for (const ShiftKind shift_kind : shift_kinds) {
                    for (const std::uint8_t shift : shifts) {
                        ...
                        const std::uint32_t instruction = [&] {
                            if (!thumb) {
                                return (subtract ? 0xe0400000U : 0xe0800000U) |
                                       (static_cast<std::uint32_t>(layout.base) << 16) |
                                       (static_cast<std::uint32_t>(layout.destination) << 12) |
                                       (static_cast<std::uint32_t>(shift) << 7) |
                                       (shift_encoding << 5) | layout.index;
                            }
                            const std::uint32_t first_half =
                                (subtract ? 0xeba0U : 0xeb00U) | layout.base;
                            const std::uint32_t second_half =
                                (static_cast<std::uint32_t>(shift >> 2) << 12) |
                                (static_cast<std::uint32_t>(layout.destination) << 8) |
                                (static_cast<std::uint32_t>(shift & 3) << 6) |
                                (shift_encoding << 4) | layout.index;
                            return (second_half << 16) | first_half;
                        }();

                        for (const auto& input : inputs) {
                            CAPTURE(subtract, thumb, layout.destination, layout.base, layout.index,
                                    shift_encoding, shift, input.base, input.index, instruction);
                            ArmTestCallbacks callbacks;
                            callbacks.code = {
                                instruction,
                            ...
                            const std::uint32_t shifted_index = [&] {
                                if (shift_kind == ShiftKind::LogicalLeft) {
                                    return initial_regs[layout.index] << shift;
                                }
                                if (shift_kind == ShiftKind::LogicalRight) {
                                    return shift == 0 ? 0U : initial_regs[layout.index] >> shift;
                                }
                                const std::uint8_t effective_shift = shift == 0 ? 31 : shift;
                                return static_cast<std::uint32_t>(
                                    static_cast<std::int32_t>(initial_regs[layout.index]) >>
                                    effective_shift);
                            }();
                            const std::uint32_t expected =
                                subtract ? initial_regs[layout.base] - shifted_index
                                         : initial_regs[layout.base] + shifted_index;
                            jit.Regs() = initial_regs;
...
TEST_CASE("Dynarmic A32 flag-setting shifted arithmetic preserves NZCV and comparison registers",
          "[core][arm][dynarmic][shifted-flags]") {
    ...
    struct ArithmeticOperation {
        ArithmeticKind kind;
        std::uint32_t arm_encoding;
        std::uint16_t thumb_encoding;
        bool subtract;
        bool writes_result;
    };
    constexpr std::array operations{
        ArithmeticOperation{ArithmeticKind::Add, 0xe0900000, 0xeb10, false, true},
        ArithmeticOperation{ArithmeticKind::Sub, 0xe0500000, 0xebb0, true, true},
        ArithmeticOperation{ArithmeticKind::Cmn, 0xe1700000, 0xeb10, false, false},
        ArithmeticOperation{ArithmeticKind::Cmp, 0xe1500000, 0xebb0, true, false},
    };
    ...
    for (const auto& operation : operations) {
        const std::size_t layout_count = operation.writes_result ? layouts.size() : 2;
        for (const bool thumb : {false, true}) {
            for (std::size_t layout_index = 0; layout_index < layout_count; ++layout_index) {
                ...
                for (const ShiftKind shift_kind : shift_kinds) {
                    for (const std::uint8_t shift : shifts) {
                        ...
                        const std::uint32_t instruction = [&] {
                            if (!thumb) {
                                return operation.arm_encoding |
                                       (static_cast<std::uint32_t>(layout.base) << 16) |
                                       (operation.writes_result
                                            ? static_cast<std::uint32_t>(layout.destination) << 12
                                            : 0) |
                                       (static_cast<std::uint32_t>(shift) << 7) |
                                       (shift_encoding << 5) | layout.index;
                            }
                            const std::uint32_t first_half = operation.thumb_encoding | layout.base;
                            const std::uint32_t destination =
                                operation.writes_result ? layout.destination : 15;
                            const std::uint32_t second_half =
                                (static_cast<std::uint32_t>(shift >> 2) << 12) |
                                (destination << 8) |
                                (static_cast<std::uint32_t>(shift & 3) << 6) |
                        ...
                        for (const auto& input : inputs) {
                            CAPTURE(static_cast<std::uint8_t>(operation.kind), thumb,
                                    layout.destination, layout.base, layout.index, shift_encoding,
                                    shift, input.base, input.index, instruction);
                            ArmTestCallbacks callbacks;
                            callbacks.code = {
                                instruction,
                                thumb ? 0xe7fee7fe : 0xeafffffe, // B .
                            ...
                            initial_regs[layout.index] =
                                layout.base == layout.index ? input.base : input.index;
                            const std::uint32_t base = initial_regs[layout.base];
                            const std::uint32_t index = initial_regs[layout.index];
                            const std::uint32_t shifted_index = [&] {
                                if (shift_kind == ShiftKind::LogicalLeft) {
                                    return index << shift;
                                }
                                if (shift_kind == ShiftKind::LogicalRight) {
                                    return shift == 0 ? 0U : index >> shift;
                                }
                                const std::uint8_t effective_shift = shift == 0 ? 31 : shift;
                                return static_cast<std::uint32_t>(
                                    static_cast<std::int32_t>(index) >> effective_shift);
                            }();
                            const std::uint32_t expected =
                                operation.subtract ? base - shifted_index : base + shifted_index;
                            const bool carry = operation.subtract
                                                 ? base >= shifted_index
                                                 : static_cast<std::uint64_t>(base) + shifted_index >
                                                       0xffffffffULL;
                            const bool overflow = operation.subtract
                                                    ? ((base ^ shifted_index) & (base ^ expected) &
                                                       0x80000000U) != 0
                                                    : (~(base ^ shifted_index) & (base ^ expected) &
                                                       0x80000000U) != 0;
                            const std::uint32_t expected_flags =
                                ...
                                (carry ? 0x20000000U : 0) |
                                (overflow ? 0x10000000U : 0) |
                                (preserved_flags & 0x080f0000U);
                            ...
                            for (std::size_t reg = 0; reg < 15; ++reg) {
                                if (operation.writes_result && reg == layout.destination) {
                                    CHECK(jit.Regs()[reg] == expected);
                                } else {
                                    CAPTURE(reg);
                                    CHECK(jit.Regs()[reg] == initial_regs[reg]);
                                }
...
TEST_CASE("Dynarmic A32 shifted logical operations preserve aliases, flags, and full-width values",
          "[core][arm][dynarmic]") {
    ...
    constexpr std::array operations{
        LogicalOperation{LogicalKind::And, 0xe0000000, 0xea00},
        LogicalOperation{LogicalKind::Eor, 0xe0200000, 0xea80},
        LogicalOperation{LogicalKind::Or, 0xe1800000, 0xea40},
    };
    ...
    for (const auto& operation : operations) {
        for (const bool set_flags : {false, true}) {
            const std::size_t layout_count = set_flags ? 2 : layouts.size();
                    ...
                    for (const ShiftKind shift_kind : shift_kinds) {
                        for (const std::uint8_t shift : shifts) {
                            ...
                            const std::uint32_t instruction = [&] {
                                ...
                                const std::uint32_t first_half =
                                    operation.thumb_encoding | (set_flags ? 0x10 : 0) |
                                    layout.base;
                                const std::uint32_t second_half =
                                    (static_cast<std::uint32_t>(shift >> 2) << 12) |
                                    (static_cast<std::uint32_t>(layout.destination) << 8) |
                            ...
                            for (std::size_t input_index = 0; input_index < input_count;
                                ...
                                CAPTURE(static_cast<std::uint8_t>(operation.kind), set_flags, thumb,
                                        layout.destination, layout.base, layout.index,
                                        shift_encoding, shift, input.base, input.index, instruction);
                                ArmTestCallbacks callbacks;
                                callbacks.code = {
                                    instruction,
                                ...
                                const std::uint32_t shifted_index = [&] {
                                    if (shift_kind == ShiftKind::LogicalLeft) {
                                        return shift == 0 ? index : index << shift;
                                    }
                                    if (shift_kind == ShiftKind::LogicalRight) {
                                        return shift == 0 ? 0U : index >> shift;
                                    }
                                    if (shift_kind == ShiftKind::ArithmeticRight) {
                                        const std::uint8_t effective_shift =
                                            shift == 0 ? 31 : shift;
                                        return static_cast<std::uint32_t>(
                                            static_cast<std::int32_t>(index) >> effective_shift);
                                    }
                                    if (shift == 0) {
                                        return 0x80000000U | (index >> 1); // RRX with C=1.
                                    }
                                    return (index >> shift) | (index << (32 - shift));
                                }();
                                const std::uint32_t expected = [&] {
                                    switch (operation.kind) {
                                    case LogicalKind::And:
                                        return initial_regs[layout.base] & shifted_index;
                                    case LogicalKind::Eor:
                                        return initial_regs[layout.base] ^ shifted_index;
                                    case LogicalKind::Or:
                                        return initial_regs[layout.base] | shifted_index;
                                    }
                                ...
                                if (set_flags) {
                                    const bool carry = [&] {
                                        ...
                                        if (shift_kind == ShiftKind::LogicalLeft) {
                                            return ((index >> (32 - shift)) & 1) != 0;
                                        }
                                        return ((index >> (shift - 1)) & 1) != 0;
                                    }();
                                    const std::uint32_t expected_flags =
                                        (expected & 0x80000000U) |
                                        (expected == 0 ? 0x40000000U : 0) |
